...
<div class="float-right right-1 absolute">
    <Button
        on:click={() => (openResourceEditorBoolean = true)}
        color="light"
        outline={false}
    ...
    >
        <svg
            ...
            class="w-5 h-5"
            fill="currentColor"
            ><title>edit-resource</title><path
    ...
    <DeleteButton resourceType={resource?.resourceType} resourceId={resource?.id} isIconButton={true} />
</div>
